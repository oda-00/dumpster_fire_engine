...
use lang_frontend::hir::{
    HirAssign, HirBtNode, HirCondition, HirEffect, HirExpr, HirScene, HirScript, IntrinsicEffect,
    IntrinsicPredicate, IntrinsicValue,
};
...
pub fn compile_to_object(
    ...
    Target::initialize_native(&InitializationConfig::default()).map_err(|e| {
        CodegenError::Llvm(std::sync::Arc::<str>::from(
            format!("target init: {e}").as_str(),
        ))
    })?;
    ...
    cg.module
        ...
        .map_err(|e| {
            CodegenError::Llvm(std::sync::Arc::<str>::from(format!("opt: {e}").as_str()))
        })?;

    cg.module.verify().map_err(|e| {
        CodegenError::Llvm(std::sync::Arc::<str>::from(format!("verify: {e}").as_str()))
    })?;

    tm.write_to_file(&cg.module, FileType::Object, out_obj)
        .map_err(|e| {
            CodegenError::Llvm(std::sync::Arc::<str>::from(
                format!("emit obj: {e}").as_str(),
            ))
        })?;
    Ok(())
}
...
impl<'ctx> Codegen<'ctx> {
    fn new(ctx: &'ctx Context, name: &str) -> Self {
        ...
        let scene_def_array_ty =
            ctx.struct_type(&[i32_ty.into(), i32_ty.into(), ptr_ty.into()], false);
    ...
    fn emit_abi_version_fn(&self) {
        ...
        let f = self
            .module
            .add_function("df_abi_version", ty, Some(Linkage::External));
    ...
    fn emit_state_size_fn(&self, state_size: u32) {
        ...
        let f = self
            .module
            .add_function("df_state_size", ty, Some(Linkage::External));
        let entry = self.ctx.append_basic_block(f, "entry");
        self.builder.position_at_end(entry);
    ...
    fn emit_state_version_fn(&self, state_version: u32) {
        ...
        let f = self
            .module
            .add_function("df_state_version", ty, Some(Linkage::External));
        let entry = self.ctx.append_basic_block(f, "entry");
        self.builder.position_at_end(entry);
    ...
    fn emit_init_state_fn(&self, hir: &HirScript) -> Result<(), CodegenError> {
        ...
        let f = self
            .module
            .add_function("df_init_state", ty, Some(Linkage::External));
        let entry = self.ctx.append_basic_block(f, "entry");
        self.builder.position_at_end(entry);
        ...
        let memset = self
            ...
            .unwrap_or_else(|| {
                ...
                self.module
                    .add_function("llvm.memset.p0.i64", memset_ty, None)
            });
    ...
    fn emit_migrate_state_fn(&self, hir: &HirScript) -> Result<(), CodegenError> {
        let ty = self.void_ty.fn_type(
            &[self.i32_ty.into(), self.ptr_ty.into(), self.ptr_ty.into()],
            false,
        );
        let f = self
            .module
            .add_function("df_migrate_state", ty, Some(Linkage::External));
        let entry = self.ctx.append_basic_block(f, "entry");
        self.builder.position_at_end(entry);
        ...
        self.builder
            .build_call(init, &[new_state.into()], "")
            .unwrap();
        ...
        if hir.migrations.is_empty() {
            ...
            for m in hir.migrations.iter() {
                let bb = self
                    .ctx
                    .append_basic_block(f, &format!("mig_{}", m.from_version));
            ...
            for (v, b) in cases.iter() {
                case_pairs.push((*v, *b));
            }
            self.builder
                .build_switch(old_version, done_bb, &case_pairs[..])
                .unwrap();
    ...
    fn emit_on_enter_fn(
        ...
        let f = self
            .module
            .add_function(&name, self.fn_void_api_state, Some(Linkage::Internal));
        let entry = self.ctx.append_basic_block(f, "entry");
        self.builder.position_at_end(entry);
    ...
    fn emit_tick_fn(
        ...
        let f = self
            .module
            .add_function(&name, self.fn_i64_api_state, Some(Linkage::Internal));
        let entry = self.ctx.append_basic_block(f, "entry");
        self.builder.position_at_end(entry);
        let api = f.get_nth_param(0).unwrap().into_pointer_value();
        let state = f.get_nth_param(1).unwrap().into_pointer_value();
        ...
        for (tidx, tr) in scene.transitions.iter().enumerate() {
            ...
            self.builder
                .build_conditional_branch(cond, take_bb, next_bb)
                .unwrap();
    ...
    fn emit_bt(
        ...
        match node {
            HirBtNode::Leaf { condition, action } => {
                ...
                let status = self
                    .builder
                    .build_alloca(self.i32_ty, "leaf_status")
                    .unwrap();
                self.builder
                    .build_store(status, self.i32_ty.const_int(BT_FAILURE as u64, true))
                    .unwrap();
                ...
                self.builder
                    .build_conditional_branch(cond_val, do_action_bb, after_bb)
                    .unwrap();
                ...
                self.builder
                    .build_store(status, self.i32_ty.const_int(BT_SUCCESS as u64, true))
                    .unwrap();
                ...
                let v = self
                    .builder
                    .build_load(self.i32_ty, status, "leaf_status_v")
                    .unwrap();
                Ok(v.into_int_value())
            }
            HirBtNode::Sequence(children) => {
                ...
                let result = self
                    .builder
                    .build_alloca(self.i32_ty, "seq_result")
                    .unwrap();
                self.builder
                    .build_store(result, self.i32_ty.const_int(BT_SUCCESS as u64, true))
                    .unwrap();
                ...
                for (i, c) in children.iter().enumerate() {
                    ...
                    self.builder
                        .build_conditional_branch(is_success, cont_bb, exit_bb)
                        .unwrap();
                ...
                let v = self
                    .builder
                    .build_load(self.i32_ty, result, "seq_v")
                    .unwrap();
                Ok(v.into_int_value())
            }
            HirBtNode::Selector(children) => {
                ...
                let result = self
                    .builder
                    .build_alloca(self.i32_ty, "sel_result")
                    .unwrap();
                self.builder
                    .build_store(result, self.i32_ty.const_int(BT_FAILURE as u64, true))
                    .unwrap();
                ...
                for (i, c) in children.iter().enumerate() {
                    ...
                    self.builder
                        .build_conditional_branch(is_failure, cont_bb, exit_bb)
                        .unwrap();
                    self.builder.position_at_end(cont_bb);
                }
                self.builder.build_unconditional_branch(exit_bb).unwrap();
                ...
                let v = self
                    .builder
                    .build_load(self.i32_ty, result, "sel_v")
                    .unwrap();
                Ok(v.into_int_value())
            }
            HirBtNode::Parallel(children) => {
                ...
                self.builder
                    .build_store(success_count, self.i32_ty.const_zero())
                    .unwrap();
                self.builder
                    .build_store(failure_count, self.i32_ty.const_zero())
                    .unwrap();
                ...
                for c in children.iter() {
                    ...
                    let cur_s = self
                        .builder
                        .build_load(self.i32_ty, success_count, "cur_s")
                        .unwrap()
                        .into_int_value();
                    let inc_s = self
                        .builder
                        .build_int_add(cur_s, self.i32_ty.const_int(1, false), "inc_s")
                        .unwrap();
                    let next_s = self
                        .builder
                        .build_select(is_succ, inc_s, cur_s, "next_s")
                        .unwrap();
                    ...
                    let cur_f = self
                        .builder
                        .build_load(self.i32_ty, failure_count, "cur_f")
                        .unwrap()
                        .into_int_value();
                    let inc_f = self
                        .builder
                        .build_int_add(cur_f, self.i32_ty.const_int(1, false), "inc_f")
                        .unwrap();
                    let next_f = self
                        .builder
                        .build_select(is_fail, inc_f, cur_f, "next_f")
                        .unwrap();
                ...
                let s_val = self
                    .builder
                    .build_load(self.i32_ty, success_count, "succ_v")
                    .unwrap()
                    .into_int_value();
                let f_val = self
                    .builder
                    .build_load(self.i32_ty, failure_count, "fail_v")
                    .unwrap()
                    .into_int_value();

                let all_succ = self
                    .builder
                    .build_int_compare(IntPredicate::EQ, s_val, total, "all_succ")
                    .unwrap();
                let any_fail = self
                    .builder
                    .build_int_compare(
            ...
            HirBtNode::Guard { cond, child } => {
                ...
                self.builder
                    .build_conditional_branch(c, do_bb, exit_bb)
                    .unwrap();
                ...
                let v = self
                    .builder
                    .build_load(self.i32_ty, result, "guard_v")
                    .unwrap()
                    .into_int_value();
                Ok(v)
            }
    ...
    fn emit_condition(
        ...
        Ok(match c {
            ...
            HirCondition::Cmp(a, op, b) => {
                ...
                self.builder
                    .build_float_compare(pred, va, vb, "cmp")
                    .unwrap()
            }
        ...
        match kind {
            IntrinsicPredicate::EnemyInRange | IntrinsicPredicate::SeePlayer => {
                ...
                let actor_count = self
                    .load_api_field(api, API_OFF_ACTOR_COUNT, self.i32_ty.into())
                    .into_int_value();
                let has_actors = self
                    .builder
                    .build_int_compare(
            ...
            IntrinsicPredicate::AfterSeconds => {
                let elapsed = self
                    .load_api_field(api, API_OFF_ELAPSED, self.f32_ty.into())
                    .into_float_value();
                let elapsed64 = self
                    .builder
                    .build_float_ext(elapsed, self.f64_ty, "e64")
                    .unwrap();
                ...
                Ok(self
                    .builder
                    .build_float_compare(FloatPredicate::OGE, elapsed64, t, "after")
                    .unwrap())
            }
    ...
    fn emit_effect(
        ...
        match e {
            HirEffect::CueTroupe(troupe_id) => {
                let cue_fn_ptr = self
                    .load_api_field(api, API_OFF_CUE_TROUPE, self.ptr_ty.into())
                    .into_pointer_value();
                let cue_fn_ty = self
                    .void_ty
                ...
                self.builder
                    .build_indirect_call(cue_fn_ty, cue_fn_ptr, &[api.into(), id.into()], "")
                    .unwrap();
            }
            HirEffect::Intrinsic(kind, args) => {
                ...
                let abi_alloca = self
                    .builder
                    .build_alloca(self.effect_abi_ty, "effect_abi")
                    .unwrap();
                ...
                self.builder
                    .build_store(abi_alloca, self.i8_ty.const_int(kind_byte as u64, false))
                    .unwrap();
                ...
                let push_fn_ptr = self
                    .load_api_field(api, API_OFF_PUSH_EFFECT, self.ptr_ty.into())
                    .into_pointer_value();
                let push_fn_ty = self
                    .void_ty
    ...
    fn emit_expr(
        ...
        match e {
            ...
            HirExpr::OldStateLoad { offset, ty } => {
                let p = old_state
                    .ok_or_else(|| CodegenError::Llvm("old.<field> used outside migrate".into()))?;
            ...
            HirExpr::Intrinsic(kind, _args) => match (kind, api) {
                (IntrinsicValue::TickCount, Some(api)) => {
                    let tc = self
                        .load_api_field(api, API_OFF_TICK_COUNT, self.i64_ty.into())
                        .into_int_value();
                    Ok(tc.into())
                }
                (IntrinsicValue::Elapsed, Some(api)) => {
                    ...
                    Ok(self
                        .builder
                        .build_float_ext(e, self.f64_ty, "el_f")
                        .unwrap()
                        .into())
                }
    ...
    fn emit_expr_as_f64(
        ...
        match e {
            ...
            HirExpr::Neg(inner) => {
                let v = self.emit_expr_as_f64(inner, state, old_state, _hir, api)?;
                Ok(self.builder.build_float_neg(v, "neg").unwrap())
            }
            HirExpr::Bin(a, op, b) => {
                let va = self.emit_expr_as_f64(a, state, old_state, _hir, api)?;
                let vb = self.emit_expr_as_f64(b, state, old_state, _hir, api)?;
                Ok(match op {
                    BinOp::Add => self.builder.build_float_add(va, vb, "add").unwrap(),
                    BinOp::Sub => self.builder.build_float_sub(va, vb, "sub").unwrap(),
                    BinOp::Mul => self.builder.build_float_mul(va, vb, "mul").unwrap(),
            ...
            HirExpr::Intrinsic(kind, _args) => Ok(match (kind, api) {
                (IntrinsicValue::TickCount, Some(api)) => {
                    let tc = self
                        ...
                        .into_int_value();
                    self.builder
                        .build_signed_int_to_float(tc, self.f64_ty, "tc_f")
                        .unwrap()
                }
                (IntrinsicValue::Elapsed, Some(api)) => {
                    let e = self
    ...
    fn to_f64(&self, v: BasicValueEnum<'ctx>, ty: Ty) -> FloatValue<'ctx> {
        match ty {
            ...
            Ty::I32 | Ty::Bool => self
                .builder
                .build_signed_int_to_float(v.into_int_value(), self.f64_ty, "i2f")
                .unwrap(),
            Ty::ActorHandle | Ty::SceneId => self
                .builder
                .build_signed_int_to_float(v.into_int_value(), self.f64_ty, "i2f")
                .unwrap(),
        }
        ...
        let stored: BasicValueEnum = match ty {
            Ty::I32 | Ty::Bool => {
                ...
                if let BasicValueEnum::FloatValue(f) = val {
                    self.builder
                        .build_float_to_signed_int(f, self.i32_ty, "f2i")
                        .unwrap()
                        .into()
                } else if let BasicValueEnum::IntValue(i) = val {
                    if i.get_type().get_bit_width() != 32 {
                        self.builder
                            .build_int_truncate_or_bit_cast(i, self.i32_ty, "trunc")
                            .unwrap()
                            .into()
                    } else {
                        i.into()
            ...
            Ty::F64 => {
                if let BasicValueEnum::IntValue(i) = val {
                    ...
                    if f.get_type() == self.f32_ty {
                        self.builder
                            .build_float_ext(f, self.f64_ty, "f2f64")
                            .unwrap()
                            .into()
                    } else {
                        f.into()
            ...
            Ty::ActorHandle | Ty::SceneId => {
                ...
                if let BasicValueEnum::FloatValue(f) = val {
                    ...
                    if i.get_type().get_bit_width() != 64 {
                        self.builder
                            .build_int_s_extend_or_bit_cast(i, self.i64_ty, "sext")
                            .unwrap()
                            .into()
                    } else {
                        i.into()
    ...
    fn emit_create_scene_defs_fn(&self, scene_count: u32) {
        ...
        let f = self
            .module
            .add_function("df_create_scene_defs", ty, Some(Linkage::External));
        let entry = self.ctx.append_basic_block(f, "entry");
        self.builder.position_at_end(entry);
        ...
        self.builder
            .build_store(count_ptr, self.i32_ty.const_int(scene_count as u64, false))
            .unwrap();
        ...
        let table = self
            .module
            .get_global("scene_entries")
            .expect("scene table missing");
        self.builder
            .build_store(scenes_ptr_ptr, table.as_pointer_value())
            .unwrap();
