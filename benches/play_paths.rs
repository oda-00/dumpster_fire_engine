...
fn build_flat_script(
    ...
    let actives = [ActiveActor::new(lh, sh, ah, aid)];
...
fn bench_handle_for(c: &mut Criterion) {
    ...
    for &n in &[16usize, 256, 4096] {
        ...
        g.bench_with_input(
            BenchmarkId::new("range_compressed", n),
            &queries,
            |b, queries| {
                b.iter(|| {
                    let mut hits = 0u64;
                    for &q in queries {
                        if play.handle_for(black_box(q)).is_some() {
                            hits += 1;
                        }
                    }
                    hits
        ...
        g.bench_with_input(
            BenchmarkId::new("hashmap_control", n),
            &queries,
            |b, queries| {
                b.iter(|| {
                    let mut hits = 0u64;
                    for &q in queries {
                        if map.contains_key(&black_box(q.raw())) {
                            hits += 1;
                        }
                    }
                    hits
