...
fn world_full_tick_xlarge() {
    ...
    for li in 0..n_levels {
        let lh = if li == 0 { primary_lh }
                 else { world.spawn_level(LevelId::new(li as i64 + 1), format!("L{li}")) };
        for _si in 0..n_stages {
            let sh = world.spawn_stage(lh, StageId::new(stage_id_ctr), "S").unwrap();
            stage_id_ctr += 1;
            let actors: Vec<_> = (0..per_stage).map(|_| {
                let aid = ActorId::new(actor_id_ctr); actor_id_ctr += 1;
                let ah = world.spawn_actor(lh, sh, aid, Affine3A::IDENTITY).unwrap();
                let cvi = world.spawn_sub_entity(lh, sh, ah,
    ...
    for (idx, &(lh, sh, ref actors)) in stage_handles.iter().enumerate() {
        ...
        let troupe   = TroupeId::new(1);
        let actives: thin_vec::ThinVec<ActiveActor> = actors.iter().enumerate()
        ...
        let make_scene = |id: SceneId, target: SceneId| SceneDef {
            ...
            transitions: thin_vec![Transition {
                condition: Condition::Always, target, effects: std::sync::Arc::default(),
            }],
        };
        let root = SceneDef {
    ...
    for _ in 0..120 { world.tick(1.0 / 60.0); } // warmup outside timing window

    // IAI measures only this single call:
    black_box(world.tick(1.0 / 60.0));
}
