...
impl MaterialExtBlock {
    ...
    pub fn from_material(m: &Material) -> Self {
        ...
        Self {
            clearcoat: [cc.factor, cc.roughness_factor, cc.normal_scale, 0.0],
            sheen: [
                sh.color_factor[0],
                sh.color_factor[1],
                sh.color_factor[2],
            ...
            iridescence: [
                ir.factor,
                ir.ior,
                ir.thickness_minimum,
                ir.thickness_maximum,
            ],
            anisotropy: [an.strength, an.rotation, m.dispersion, 0.0],
            diff_trans: [
                dt.factor,
                dt.color_factor[0],
                dt.color_factor[1],
...
fn vec3_field(o: &Map<String, Value>, key: &str, default: [f32; 3]) -> [f32; 3] {
    let Some(arr) = o.get(key).and_then(|v| v.as_array()) else {
        return default;
    };
    let mut out = default;
...
pub fn parse_material_extensions(out: &mut Material, extensions: &Map<String, Value>) {
    if let Some(o) = extensions
        .get("KHR_materials_clearcoat")
        .and_then(|v| v.as_object())
    {
        // Default factor is 0.0 per spec; default roughness 0.0.
        out.clearcoat = Some(Clearcoat {
            factor: f32_field(o, "clearcoatFactor", 0.0),
            roughness_factor: f32_field(o, "clearcoatRoughnessFactor", 0.0),
            texture: texture_ref_field(o, "clearcoatTexture"),
            roughness_texture: texture_ref_field(o, "clearcoatRoughnessTexture"),
            normal_texture: texture_ref_field(o, "clearcoatNormalTexture"),
            normal_scale: o
    ...
    if let Some(o) = extensions
        .get("KHR_materials_volume_scatter")
        .and_then(|v| v.as_object())
    {
