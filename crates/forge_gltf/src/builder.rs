...
pub fn build_test_glb(
    ...
    let norm_acc: Option<usize> = norm_bv.map(|(off, len)| {
        ...
        let ac = accs.len();
        accs.push(format!(
            r#"{{"bufferView":{bv},"componentType":5126,"count":{cnt},"type":"VEC3"}}"#,
    ...
    let uv_acc: Option<usize> = uv_bv.map(|(off, len)| {
        ...
        let ac = accs.len();
        accs.push(format!(
            r#"{{"bufferView":{bv},"componentType":5126,"count":{cnt},"type":"VEC2"}}"#,
            cnt = uvs.unwrap().len()
        ));
        ac
    });
    ...
    let idx_acc: Option<usize> = idx_bv.map(|(off, len)| {
        ...
        let ac = accs.len();
        accs.push(format!(
            r#"{{"bufferView":{bv},"componentType":5125,"count":{cnt},"type":"SCALAR"}}"#,
    ...
    if let Some(i) = norm_acc {
        attrs.push_str(&format!(r#","NORMAL":{i}"#));
    }
    if let Some(i) = uv_acc {
        attrs.push_str(&format!(r#","TEXCOORD_0":{i}"#));
    }
