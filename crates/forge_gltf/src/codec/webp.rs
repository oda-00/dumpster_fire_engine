...
fn dispatch_webp(riff: WebpRiff<'_>) -> GltfResult<(u32, u32, ThinVec<u8>)> {
    ...
    if let Some(data) = vp8l {
        ...
        Err(GltfError::UnsupportedFeature(
            "WebP: no supported image chunk (VP8/VP8L)".to_owned(),
        ))
    }
}
...
impl HuffTree {
    ...
    fn build(lengths: &[u8], num_symbols: usize) -> GltfResult<Self> {
        ...
        let nonzero: Vec<(usize, u8)> = lengths
            .iter()
            .take(num_symbols)
            .enumerate()
            .filter(|&(_, &l)| l > 0)
        ...
        for (sym, &l) in lengths[..num_symbols].iter().enumerate() {
            if l == 0 {
                continue;
            }
    ...
    fn decode(&self, br: &mut BitReader<'_>) -> GltfResult<u16> {
        ...
        if br.bits_in == 0 {
            return Err(GltfError::InvalidAccessor(
                "WebP: bitstream truncated in Huffman decode",
            ));
        }
        ...
        for &(code_msb, clen, csym) in &self.codes {
            if (clen as u32) <= self.table_bits {
                continue;
            }
...
fn read_huffman_tree(br: &mut BitReader<'_>, alphabet_size: usize) -> GltfResult<HuffTree> {
    ...
    while i < alphabet_size {
        ...
        match sym {
            ...
            _ => {
                return Err(GltfError::InvalidAccessor(
                    "WebP VP8L: invalid cl_tree symbol",
                ));
            }
        }
    }
...
fn decode_vp8l(data: &[u8]) -> GltfResult<(u32, u32, ThinVec<u8>)> {
    if data.is_empty() || data[0] != 0x2f {
        return Err(GltfError::InvalidAccessor(
            "WebP VP8L: missing signature byte 0x2f",
        ));
    }
    ...
    while br.read_bit()? == 1 {
        ...
        match ttype {
            ...
            3 => {
                let index_bits_raw = br.read_bits(8)?; // palette_size - 1
                ...
                let index_bits = if palette_size <= 2 {
                    1u32
            ...
            _ => {
                return Err(GltfError::InvalidAccessor(
                    "WebP VP8L: unknown transform type",
                ));
            }
        }
        if transforms.len() > 4 {
    ...
    for &argb in pixels.iter().take(num_pixels) {
...
fn decode_vp8l_image(
    ...
    while px + py * w < num_pixels {
        ...
        if green < 256 {
            ...
            let val = if cache_idx < cache_size {
                color_cache[cache_idx]
            } else {
                0
            };
            pixels[idx] = val;
...
fn effective_dims(transforms: &[Vp8lTransform], width: u32) -> (u32, bool) {
    for t in transforms.iter().rev() {
            ...
            && *index_bits < 8
        {
            ...
            let eff = width.div_ceil(pix_per_byte);
            return (eff, true);
        }
    }
    (width, false)
...
fn apply_inverse_transform(
    ...
    match t {
        ...
        Vp8lTransform::Color { size_bits, meta } => {
            #[cfg(target_arch = "x86_64")]
            unsafe {
                Ok(color_transform_sse2(
                    pixels, *size_bits, meta, width, height,
                ))
            }
            #[cfg(not(target_arch = "x86_64"))]
            {
                ...
                for y in 0..h {
                    for x in 0..w {
                        ...
                        out[y * w + x] =
                            (a << 24) | ((new_r as u32) << 16) | ((g as u32) << 8) | new_b as u32;
                    }
                }
                Ok(out)
            }
        ...
        Vp8lTransform::Predictor { size_bits, meta } => {
            #[cfg(target_arch = "x86_64")]
            unsafe {
                Ok(predictor_transform_sse2(
                    pixels, width, height, *size_bits, meta,
                ))
            }
            #[cfg(not(target_arch = "x86_64"))]
            {
        ...
        for y in 0..h {
            ...
            while x < w {
                ...
                while xi + 4 <= cell_end {
                    ...
                    let r_add_lo32 =
                        _mm_slli_epi32(_mm_cvtepu8_to_epi32_sse2(r_delta_lo_packed), 16);
                    let r_add_hi32 =
                        _mm_slli_epi32(_mm_cvtepu8_to_epi32_sse2(r_delta_hi_packed), 16);
...
unsafe fn predictor_transform_sse2(
    ...
    unsafe {
        ...
        for y in 1..h {
            ...
            {
                let val = if row_base < pixels.len() {
                    pixels[row_base]
                } else {
                    0
                };
            ...
            while x < w {
                let mx = x >> size_bits;
                let meta_idx = my * mw + mx;
                let pred_mode = if meta_idx < meta.len() {
                    (meta[meta_idx] >> 8) & 0xff
                } else {
                    0
                };
                ...
                if l_dependent {
                    for xi in x..cell_end {
                        let val = if row_base + xi < pixels.len() {
                            pixels[row_base + xi]
                        } else {
                            0
                        };
                        ...
                        let pred_v = match pred_mode {
                            ...
                            3 => {
                                ...
                                if xi + 5 <= w {
                                    ...
                                    for xj in xi..(xi + 4).min(cell_end) {
                                        let val = if row_base + xj < pixels.len() {
                                            pixels[row_base + xj]
                                        } else {
                                            0
                                        };
                            ...
                            9 => {
                                ...
                                if xi + 5 <= w {
                                    let p_t = out.as_ptr().add(top_base + xi) as *const __m128i;
                                    let p_tr =
                                        out.as_ptr().add(top_base + xi + 1) as *const __m128i;
                                    let t = _mm_loadu_si128(p_t);
...
fn clamp_add_sub_full(a: u32, b: u32, c: u32) -> u32 {
    argb_channels(a, b, c, |av, bv, cv| {
        (av as i32 + bv as i32 - cv as i32).clamp(0, 255) as u32
    })
}
...
fn decode_vp8(data: &[u8]) -> GltfResult<(u32, u32, ThinVec<u8>)> {
    ...
    if data.len() < 10 {
        return Err(GltfError::InvalidAccessor(
            "VP8: key frame header too short",
        ));
    }
    ...
    let y2_dc_q = vp8_dc_quant(base_q_idx + y2_dc_delta) * 2;
...
static VP8_DC_QUANT: [i32; 128] = [
    ...
    93, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113,
