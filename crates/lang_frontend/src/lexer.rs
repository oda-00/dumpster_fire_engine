...
impl<'src> Lexer<'src> {
    ...
    fn lex_string(&mut self) -> Result<TokenKind, LexError> {
        ...
        loop {
            match self.advance() {
                ...
                Some(b'\\') => match self.advance() {
                    ...
                    _ => {
                        return Err(LexError {
                            msg: Arc::from("invalid escape sequence"),
                            line,
                            col,
                        });
                    }
                ...
                None => {
                    return Err(LexError {
                        msg: Arc::from("unterminated string"),
                        line,
                        col,
                    });
                }
        ...
        let text = core::str::from_utf8(&buf).map_err(|_| LexError {
            msg: Arc::from("non-utf8 string"),
            line,
            col,
        })?;
...
mod tests {
    ...
    fn literals_and_arrows() {
        ...
        assert!(matches!(t[1].kind, TokenKind::FloatLit(f) if (f - 3.14_f64).abs() < 1e-6));
    ...
    fn string_with_escapes() {
        let t = lex(r#""hello\n\tworld""#);
        let TokenKind::StringLit(ref s) = t[0].kind else {
            panic!()
        };
