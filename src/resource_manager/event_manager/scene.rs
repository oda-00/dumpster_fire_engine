...
pub enum Effect {
    ...
    UiAction { action: u32, payload: f32 },
}
...
impl Clone for Effect {
    fn clone(&self) -> Self {
        match self {
            ...
            Effect::AddComponent(b) => Effect::AddComponent(Arc::new(AddComponentEffect {
                level_h: b.level_h,
                stage_h: b.stage_h,
                actor_h: b.actor_h,
            ...
            Effect::UiAction { action, payload } => Effect::UiAction {
                action: *action,
                payload: *payload,
            },
        }
...
pub fn clone_component_pub(c: &Component) -> Component {
    clone_component(c)
}
...
fn clone_shape(
    s: &crate::resource_manager::component::CollisionShape,
) -> crate::resource_manager::component::CollisionShape {
...
impl Condition {
    pub fn eval(&self, ctx: &EvalCtx<'_>) -> bool {
        match self {
            ...
            Condition::TroupeAny { troupe_idx, predicate, .. } => {
                ...
                group
                    .iter()
                    .any(|a| predicate.eval_for_actor(ctx, a.actor_id))
            }
            Condition::EventFired(id) => ctx
                .events_seen
                .iter()
                .any(|e| matches!(e, Event::Custom(eid, _) if eid == id)),
    ...
    fn eval_for_actor(&self, ctx: &EvalCtx<'_>, actor: ActorId) -> bool {
        match self {
            Condition::ActorNear { target, radius, .. } => actor_world_pos(ctx, actor)
                .map(|p| p.distance(*target) <= *radius)
                .unwrap_or(false),
...
fn actor_world_pos(ctx: &EvalCtx<'_>, id: ActorId) -> Option<Vec3> {
    ...
    if !stage.actors.contains(active.actor_h) {
        return None;
    }
...
fn actor_has_component(ctx: &EvalCtx<'_>, id: ActorId, ct: ComponentType) -> bool {
    let Some(active) = ctx.actors.iter_all().find(|a| a.actor_id == id) else {
        return false;
    };
    let Some(stage) = ctx
        .world
        .levels
        .get(active.level_h)
        .and_then(|l| l.stages.get(active.stage_h))
    else {
        return false;
    };
    let Some(actor) = stage.actors.get(active.actor_h) else {
        return false;
    };
    actor
        .sub_entities
...
impl Clone for BtNode {
    fn clone(&self) -> Self {
        match self {
            ...
            BtNode::Parallel { children, policy } => BtNode::Parallel {
            ...
            BtNode::Decorator { decorator, child } => BtNode::Decorator {
        ...
        match self {
            ...
            BtNode::Leaf(op) => {
                ...
                if op.condition.eval(ctx) {
                    ...
                    if op.once {
                        op.fired.store(true, Ordering::Relaxed);
                    }
                    BtStatus::Success
                } else {
                    BtStatus::Running
...
pub enum SceneKind {
    Atomic,
    Compound {
        children: ThinVec<SceneId>,
        initial: SceneId,
        history: Option<SceneId>,
    },
    AndParallel {
        regions: ThinVec<Region>,
    },
}
