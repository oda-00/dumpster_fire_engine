...
impl Default for LayoutSpec {
    fn default() -> Self {
        Self { axis: Axis::Column, gap: 4.0, padding: Padding::default(), align: Align::Start }
    }
}
...
pub fn measure_and_place(
    ...
    let main_axis_extent = match spec.axis { Axis::Row => inner_w, Axis::Column => inner_h };
    ...
    let fill_each = if fill_count > 0 { remaining / fill_count as f32 } else { 0.0 };
    ...
    for (i, s) in sizes.iter().enumerate() {
        ...
        let r = match spec.axis {
            Axis::Row    => Rect { x: inner_x + cursor_main, y: inner_y, w: main, h: inner_h },
            Axis::Column => Rect { x: inner_x, y: inner_y + cursor_main, w: inner_w, h: main },
        };
        out.push(r);
