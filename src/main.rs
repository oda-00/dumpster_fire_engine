...
fn main() {
    ...
    let sh = world
        .spawn_stage(lh, StageId::new(1), "default_stage")
        .unwrap();
    ...
    println!(
        ...
        stage_ref.worlds[ah.idx as usize].translation
    );
    println!();
    ...
    if let Some(bobby) = &actor.sub_entities[bobby_id.variant_idx()] {
        ...
        println!(
            "Bobby has Physics?    {}",
            bobby.has_component(ComponentType::Physics)
        );
        println!(
            "Bobby has Audio?      {}",
            bobby.has_component(ComponentType::Audio)
        );
        println!();
    }
    ...
    let actor = &world.levels[lh].stages[sh].actors[ah];
    println!(
        "After moving Bobby:   world pos {:?}",
        actor.sub_entities[bobby_vi]
            .as_ref()
            .unwrap()
            .world
            .translation
    ...
    println!(
        "Removed from Bobby:   {:?}",
        removed.map(|_| ComponentType::Transform)
    );
    let actor = &world.levels[lh].stages[sh].actors[ah];
    println!(
        "Bobby has Transform?  {}",
    ...
    let act1 = SceneDef {
        ...
        initial_actors: thin_vec![thin_vec![ActiveActor::new(lh, sh, ah, ActorId::new(1)),]],
        root: act1_bt,
        on_enter: thin_vec![],
        on_exit: thin_vec![],
        handlers: thin_vec![],
    ...
    for tick_n in 0..150 {
        ...
        let leaves: ThinVec<i64> = world.levels[lh].stages[sh]
            .play
            .as_ref()
            .map(|p| {
                p.active_leaves
