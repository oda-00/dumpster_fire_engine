...
impl AppLogic for HelloGltfApp {
    fn on_start(&mut self, ctx: &mut AppCtx<'_>, ev: &ActiveEventLoop) -> ForgeResult<()> {
        ...
        // Key light.
        let light_ah = self.world.spawn_actor(
            lh, sh, ActorId::new(1),
        ...
        let mesh_ah = self.world.spawn_actor(lh, sh, ActorId::new(2), Affine3A::IDENTITY).unwrap();
        self.world.spawn_sub_entity(lh, sh, mesh_ah,
