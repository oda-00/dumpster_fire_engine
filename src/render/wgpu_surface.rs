...
fn ui_vertex_layout() -> wgpu::VertexBufferLayout<'static> {
    ...
    wgpu::VertexBufferLayout {
        ...
        attributes:   &[
            wgpu::VertexAttribute { shader_location: 0, offset: 0,  format: Float32x2 }, // pos
            wgpu::VertexAttribute { shader_location: 1, offset: 8,  format: Float32x2 }, // uv
            wgpu::VertexAttribute { shader_location: 2, offset: 16, format: Unorm8x4  }, // color
        ],
    }
}
...
impl WgpuSurface {
    pub fn new(
        ...
        let font_tex = device.create_texture(&wgpu::TextureDescriptor {
            ...
            size:            wgpu::Extent3d { width: font::ATLAS_W, height: font::ATLAS_H, depth_or_array_layers: 1 },
            mip_level_count: 1,
            sample_count:    1,
        ...
        queue.write_texture(
            ...
            wgpu::Extent3d { width: font::ATLAS_W, height: font::ATLAS_H, depth_or_array_layers: 1 },
        );
        ...
        let tonemap_param_bgl = uniform_bgl(&device, "tonemap-param-bgl", wgpu::ShaderStages::FRAGMENT);
        ...
        let tonemap_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            ...
            contents: unsafe { std::slice::from_raw_parts(&tonemap_init as *const u32 as *const u8, 4) },
            usage:    wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });
        ...
        let ui_shader_src = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"), "/assets/shaders/ui.wgsl"
        ));
        let tm_shader_src = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"), "/assets/shaders/tonemap.wgsl"
        ));
        let sc_shader_src = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"), "/assets/shaders/forward_lit_simple.wgsl"
        ));
        ...
        Ok(Self {
            ...
            hdr_tex, hdr_view, depth_tex, depth_view,
            font_tex, font_view, sampler,
            screen_buf, screen_bind_group,
            tonemap_buf, tonemap_bind_group,
            ui_pipeline, tonemap_pipeline, scene_pipeline,
            atlas_bind_group, hdr_bind_group,
            ui_vb, ui_ib,
...
impl GpuSurface for WgpuSurface {
    fn backend(&self) -> BackendKind { BackendKind::Wgpu }
    ...
    fn draw_frame(
        ...
        self.queue.write_buffer(&self.tonemap_buf, 0,
            unsafe { std::slice::from_raw_parts(&tonemap_val as *const u32 as *const u8, 4) });
    ...
    fn resize(&mut self, width: u32, height: u32) {
        ...
        let tonemap_tex_bgl = self.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        ...
        let screen_data: [f32; 2] = [width as f32, height as f32];
        self.queue.write_buffer(&self.screen_buf, 0,
            unsafe { std::slice::from_raw_parts(screen_data.as_ptr() as *const u8, 8) });
    }
}
...
fn uniform_bgl(device: &wgpu::Device, label: &str, visibility: wgpu::ShaderStages)
