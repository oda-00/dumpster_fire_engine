...
fn daemon_hot_reload_round_trip() {
    ...
    client
        .watch(1, Arc::from(src.to_string_lossy().as_ref()))
        .expect("watch");
    ...
    // Load + run via ScriptManager: on_enter emits cue_troupe("v1").
    {
        ...
        unsafe {
            (scene.on_enter)(&api, state.as_mut_ptr());
        }
...
fn state_version_of(o: &Arc<str>) -> u32 {
    ...
    let f: unsafe extern "C" fn() -> u32 = unsafe {
        obj.fn_ptr("df_state_version")
            .expect("df_state_version missing")
    };
    unsafe { f() }
}
...
fn state_size_of(o: &Arc<str>) -> u32 {
    ...
    let f: unsafe extern "C" fn() -> u32 =
        unsafe { obj.fn_ptr("df_state_size").expect("df_state_size missing") };
    unsafe { f() }
}
